(function () {
    ...
    const CSS = `
        ...
        .orderly-fms-clothing-overlay {
            position: fixed; inset: 0; background: rgba(0,0,0,0.6);
            z-index: 99999; display: flex; align-items: center; justify-content: center;
            opacity: 0; visibility: hidden; transition: all 0.3s ease;
            font-family: 'Inter', sans-serif;
        }
        .orderly-fms-clothing-overlay.open { opacity: 1; visibility: visible; }
        
        .orderly-fms-clothing-modal {
        ...
        .orderly-fms-clothing-overlay.open .orderly-fms-clothing-modal { transform: scale(1); }

        .orderly-fms-clothing-logo {
            position: absolute; top: 20px; left: 20px; z-index: 10;
            max-height: 40px; width: auto; object-fit: contain; pointer-events: none;
        }

        @media (max-width: 768px) {
            .orderly-fms-clothing-modal { grid-template-columns: 1fr; height: 100%; border-radius: 0; }
            .orderly-fms-clothing-left-panel { display: none; }
            .orderly-fms-clothing-logo { top: 15px; left: 15px; max-height: 30px; }
        }

        /* Left Panel */
        .orderly-fms-clothing-left-panel {
            background: #f3f4f6; position: relative; overflow: hidden;height: 100%;
        }
        .orderly-fms-clothing-product-img {
            width: 100%; height: 100%; object-fit: cover; mix-blend-mode: multiply; opacity: 0.9;
        }
        .orderly-fms-clothing-left-content {
        ...
        /* Right Panel */
        .orderly-fms-clothing-right-panel {
            padding: 2.5rem; display: flex; flex-direction: column; position: relative;max-height: 600px; overflow-y: auto;
        }
        
        .orderly-fms-clothing-header {
            display: flex; justify-content: space-between; align-items: center;
        }
        .orderly-fms-clothing-steps-indicator {
            font-size: 0.75rem; font-weight: 700; text-transform: uppercase; color: var(--wm-fms-clothing-primary); letter-spacing: 0.05em;
        }
        .orderly-fms-clothing-close {
            background: none; border: none; cursor: pointer; color: #9ca3af; padding: 0.5rem;
            border-radius: 50%; transition: color 0.2s;
        }
        .orderly-fms-clothing-close:hover { color: #ef4444; background: #fee2e2; }

        .orderly-fms-clothing-content { flex: 1; overflow-y: auto; padding-right: 0.5rem; }
        
        /* Titles/Text */
        .orderly-fms-clothing-h2 { font-size: 1.5rem; font-weight: 800; color: #111827;  }
        .orderly-fms-clothing-p { color: #6b7280; margin-bottom: 1.5rem; }

        /* Grid Buttons */
        .orderly-fms-clothing-grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; }
        .orderly-fms-clothing-grid-4 { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.75rem; }
        
        .orderly-fms-clothing-btn-tile {
        ...
        .orderly-fms-clothing-btn-tile:hover { border-color: var(--wm-fms-clothing-primary); background: #eff6ff; color: var(--wm-fms-clothing-primary); }
        
        .orderly-fms-clothing-list-btn {
            width: 100%; display: flex; align-items: center; justify-content: space-between;
            padding: 1rem; border: 2px solid #f3f4f6; border-radius: 1rem;
            background: white; cursor: pointer; transition: all 0.2s;
            text-align: left; margin-bottom: 0.75rem;
        }
        .orderly-fms-clothing-list-btn:hover { border-color: var(--wm-fms-clothing-primary); background: #eff6ff; }
        .orderly-fms-clothing-list-label { font-weight: 700; color: #111827; display: block; }
        .orderly-fms-clothing-list-desc { font-size: 0.875rem; color: #9ca3af; }

        /* Trigger Button Styling */
        .orderly-fms-clothing-trigger-btn {
        ...
        .orderly-fms-clothing-trigger-btn:hover { background: #e5e7eb; }

        /* Loader */
        .orderly-fms-clothing-loader {
        ...
        /* Comparison List */
        .orderly-fms-clothing-comparison {
            margin-top: 1rem;
            display: flex;
        ...
        .orderly-fms-clothing-comp-card {
            display: flex;
            align-items: center;
        ...
        .orderly-fms-clothing-comp-card.recommended {
            border-color: var(--wm-fms-clothing-primary);
            background: #eff6ff;
        }
        .orderly-fms-clothing-comp-size {
            font-size: 1.125rem;
            font-weight: 800;
            color: #111;
        }
        .orderly-fms-clothing-comp-info {
            text-align: right;
        }
        .orderly-fms-clothing-comp-status {
            font-size: 0.65rem;
            font-weight: 700;
            text-transform: uppercase;
            letter-spacing: 0.05em;
        }
        .orderly-fms-clothing-status-tight { color: #ef4444; }
        .orderly-fms-clothing-status-recommended { color: var(--wm-fms-clothing-primary); }
        .orderly-fms-clothing-status-loose { color: #6b7280; }
        .orderly-fms-clothing-comp-cm { font-size: 0.75rem; color: #9ca3af; margin-top: 2px; }
    `;
    ...
    const FindMySizeClothing = {
        ...
        init(config) {
            ...
            const savedFit = sessionStorage.getItem('orderly_fms_cloth_fit_id');
            const savedGender = sessionStorage.getItem('orderly_fms_cloth_gender');
            const savedHeight = sessionStorage.getItem('orderly_fms_cloth_height');
            const savedWeight = sessionStorage.getItem('orderly_fms_cloth_weight');
        ...
        setupTrigger() {
            ...
            container.innerHTML = `
                <button class="orderly-fms-clothing-trigger-btn" onclick="FindMySizeClothing.open()">
        ...
        createModal() {
            ...
            el.className = 'orderly-fms-clothing-overlay';
            el.innerHTML = `
                <div class="orderly-fms-clothing-modal" onclick="event.stopPropagation()">
                    ${this.config.logoUrl ? `<img src="${this.config.logoUrl}" class="orderly-fms-clothing-logo" />` : ''}
                    <div class="orderly-fms-clothing-left-panel">
                        <img src="${this.config.productImage}" class="orderly-fms-clothing-product-img">
                        <div class="orderly-fms-clothing-left-content">
                    ...
                    <div class="orderly-fms-clothing-right-panel">
                        <div class="orderly-fms-clothing-header">
                            <span id="wm-fms-clothing-step-indicator" class="orderly-fms-clothing-steps-indicator">Passo 1 de 3</span>
                            <button class="orderly-fms-clothing-close" onclick="FindMySizeClothing.close()">${ICONS.x}</button>
                        </div>
                        <div id="wm-fms-clothing-content" class="orderly-fms-clothing-content"></div>
                    </div>
                </div>
            `;
        ...
        fullReset() {
            this.reset();
            sessionStorage.removeItem('orderly_fms_cloth_fit_id');
            sessionStorage.removeItem('orderly_fms_cloth_gender');
            sessionStorage.removeItem('orderly_fms_cloth_height');
            sessionStorage.removeItem('orderly_fms_cloth_weight');
            this.render();
        },
        ...
        async fetchSizes() {
            ...
            try {
                ...
                const url = `/api/orderly/size-chart?apiKey=${this.config.apiKey}&brandId=${this.config.targetBrandId}`;
        ...
        async calculateSize() {
            ...
            sessionStorage.setItem('orderly_fms_cloth_height', height);
            sessionStorage.setItem('orderly_fms_cloth_weight', weight);
            sessionStorage.setItem('orderly_fms_cloth_gender', gender);
        ...
        render() {
            ...
            if (loading) {
                container.innerHTML = `
                    <div style="text-align:center; padding: 4rem 0;">
                        <div class="orderly-fms-clothing-loader"></div>
            ...
            if (step === 1) {
                ...
                container.innerHTML = `
                    <h2 class="orderly-fms-clothing-h2">Qual o caimento?</h2>
                    <p class="orderly-fms-clothing-p">Como você gosta que a peça fique no corpo?</p>
                    <div style="display:grid; grid-template-columns: 1fr; gap:0.5rem">
                        ${FITS.map(c => `
                            <button class="orderly-fms-clothing-list-btn" onclick="
                                FindMySizeClothing.state.fitId=${c.id}; 
                                sessionStorage.setItem('orderly_fms_cloth_fit_id', ${c.id});
                                FindMySizeClothing.fetchSizes()">
                                <div>
                                    <span class="orderly-fms-clothing-list-label">${c.label}</span>
                                    <span class="orderly-fms-clothing-list-desc">${c.desc}</span>
                                </div>
                                ${ICONS.chevronRight}
                            </button>
                        `).join('')}
                ...
                container.innerHTML = `
                    ...
                    <h2 class="orderly-fms-clothing-h2">Suas Medidas</h2>
                    <p class="orderly-fms-clothing-p">Preencha os dados abaixo para calcularmos o melhor tamanho.</p>
                ...
                container.innerHTML = `
                    <div style="text-align:center; padding:1rem 0; animation: fadeIn 0.5s; overflow-y: auto">
                        ...
                        <div class="orderly-fms-clothing-comparison">
                    ...
                    return `
                                    <div class="orderly-fms-clothing-comp-card ${item.is_recommended ? 'recommended' : ''}">
                                        <div class="orderly-fms-clothing-comp-size">
                                            ${item.size}
                                        ...
                                        <div class="orderly-fms-clothing-comp-info">
                                            <div class="orderly-fms-clothing-comp-status orderly-fms-clothing-status-${statusClass}">${statusLabel}</div>
                                        </div>
                                    </div>
                                `;
                }).join('') : ''}
