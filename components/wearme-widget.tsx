...
import { saveToCache, getFromCache } from "@/lib/orderly-storage"

import { createPortal } from "react-dom"

interface orderlyWidgetProps {
    productImage: string
    productTitle?: string
...
export function orderlyWidget({ productImage, productTitle = "Este Produto", onOpenChange }: orderlyWidgetProps) {
    ...
    const startProcessing = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/orderly/generate', {
                method: 'POST',
                body: formData
            })
    ...
    const downloadImage = () => {
        ...
        link.download = `orderly-${productTitle.replace(/\s+/g, '-').toLowerCase()}.png`
                        ...
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white/80 backdrop-blur-sm z-10 sticky top-0">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-muted-foreground font-medium">orderly AI v2.0</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setIsOpen(false)}
