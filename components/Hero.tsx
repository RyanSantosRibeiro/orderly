...
export const HeroSection = () => {
  ...
  const orderlyBtnRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    // @ts-ignore
    if (!window.orderly || !orderlyBtnRef.current) return

    // 🔥 Limpa qualquer instância anterior
    orderlyBtnRef.current.innerHTML = ""

    // (Opcional) Se o SDK tiver destroy/reset
    // window.orderly.destroy?.()

    // @ts-ignore
    window.orderly.init({
      apiKey: "023ec83bdbf73177a680807fc144bfea",
      productImage: categoryImage[selectedCategory],
      buttonSelector: "#orderly-btn" //@ts-ignore
    }, {
      resultImage: null,
      status: "idle",
      userImage: null,
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center h-auto">
        ...
        <div className="space-y-8 text-center lg:text-left">
          ...
          <p className="max-w-xl mx-auto lg:mx-0 text-lg md:text-xl text-muted-foreground font-medium leading-relaxed">
            Permita que seus clientes experimentem roupas digitalmente com apenas uma foto. Aumente as vendas e reduza devoluções com o **orderly Widget**.
          </p>
        ...
        {/* Right Column: PDP Simulation (The "orderly" Interface) */}
        <div className={`relative w-full max-w-lg mx-auto lg:max-w-none h-auto ${!out ? `animate-${selectedCategory}` : 'animate-out'} hover:rotate-1 transition-transform duration-500`}>
          ...
          <div id="orderly-container" className="orderly-container relative bg-white border border-border rounded-[2.5rem] shadow-2xl shadow-primary/10 overflow-hidden min-h-[90vh] md:min-h-[50vh] flex items-center justify-center transform hover:rotate-y-1 transition-transform duration-500">

            {/* Background: Product Page Mockup */}
            <div id="orderly-background" className={`absolute inset-0 flex flex-col transition-all duration-500 h-fit ${isModalOpen ? 'opacity-30 scale-95 blur-sm' : 'opacity-100 scale-100'}`}>
              {/* Mockup Header */}
              ...
              <div className="flex-1 p-6 flex flex-col lg:flex-row gap-6 items-center lg:items-start overflow-hidden h-auto">
                ...
                <div className="flex-1 w-full space-y-4 text-center md:text-left">
                  ...
                  <div className="pt-2">
                    <div id="orderly-btn" ref={orderlyBtnRef}></div>

                    <Script src="/orderly.js" strategy="afterInteractive" onLoad={() => {
                      // @ts-ignore
                      if (window.orderly) {
                        // @ts-ignore
                        window.orderly.init({
          ...
          <div id="orderly-floating-1" className="orderly-floating absolute -top-12 -right-12 bg-white p-4 rounded-2xl shadow-xl shadow-black/5 animate-float delay-100 hidden lg:block">
          ...
          <div id="orderly-floating-2" className="orderly-floating absolute -bottom-8 -left-8 bg-white p-4 rounded-2xl shadow-xl shadow-black/5 animate-float delay-300 hidden lg:block">
            <div className="flex items-center gap-3">
