...
export default async function SettingsPage() {
  ...
  // Fetch orderly Config
  const { data: orderlyConfig } = await supabase
    .from("wearme_configs")
    .select("*")
    .eq("owner_id", user.id)
    .single()
  ...
  return (
    <div className="space-y-10 p-8 max-w-7xl mx-auto">
      ...
      <Tabs defaultValue="profile" className="space-y-8">
        ...
        <TabsContent value="orderly" className="space-y-8 animate-in fade-in duration-500">
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-gray-50 bg-gray-50/50">
            ...
            <div className="p-8">
              <IntegrationSettings initialConfig={orderlyConfig} />
            </div>
          </div>
        </TabsContent>
      </Tabs>
